/**
 * Script para verificar por qué dos usuarios específicos no aparecen en el feed
 * 
 * Uso: npm run ts-node scripts/working/check-feed-users.ts
 */
...
async function listUsersWithPattern(client: any, pattern: string) {
  const { data: authUsers, error: authError } = await client.auth.admin.listUsers();
  ...
  return authUsers.users.filter((u: any) => 
    u.email && u.email.toLowerCase().includes(pattern.toLowerCase())
  );
}

async function main() {
  console.log('🔍 Verificando usuarios chico1 y chica1 en el feed\n');
  ...
  try {
    ...
    // First, let's find users with "chico" or "chica" in email
    console.log('🔍 Buscando usuarios con "chico" o "chica" en el email...\n');
    const chicoUsers = await listUsersWithPattern(client, 'chico');
    const chicaUsers = await listUsersWithPattern(client, 'chica');

    if (chicoUsers.length > 0) {
      console.log('Usuarios encontrados con "chico":');
      chicoUsers.forEach((u: any) => console.log(`  - ${u.email} (ID: ${u.id})`));
    }

    if (chicaUsers.length > 0) {
      console.log('\nUsuarios encontrados con "chica":');
      chicaUsers.forEach((u: any) => console.log(`  - ${u.email} (ID: ${u.id})`));
    }

    // Get users by email
    const chico1 = await getUserByEmail(client, 'chico1@example.com');
    let chica1 = await getUserByEmail(client, 'chica2@example.com');
    
    // If chica2@example.com doesn't exist, try chica1@example.com
    if (!chica1) {
      console.log('\n⚠️ chica2@example.com no encontrado, intentando chica1@example.com...');
      chica1 = await getUserByEmail(client, 'chica1@example.com');
    }

    if (!chico1) {
      console.error('❌ No se encontró chico1@example.com');
      process.exit(1);
    }

    if (!chica1) {
      console.error('❌ No se encontró chica2@example.com');
      process.exit(1);
    }

    printUserInfo(chico1, '👤 chico1');
    printUserInfo(chica1, '👤 chica1');

    // Check both directions
    console.log('\n' + '='.repeat(80));
    const result1 = await simulateFeedQuery(client, chico1, chica1);
    console.log('\n📋 Resultado:');
    result1.reasons.forEach((reason) => console.log(`  ${reason}`));
    console.log(`\n${result1.passes ? '✅' : '❌'} ${chico1.name} ${result1.passes ? 'SÍ' : 'NO'} vería a ${chica1.name} en el feed`);

    console.log('\n' + '='.repeat(80));
    const result2 = await simulateFeedQuery(client, chica1, chico1);
    console.log('\n📋 Resultado:');
    result2.reasons.forEach((reason) => console.log(`  ${reason}`));
    console.log(`\n${result2.passes ? '✅' : '❌'} ${chica1.name} ${result2.passes ? 'SÍ' : 'NO'} vería a ${chico1.name} en el feed`);

    // Summary
    console.log('\n' + '='.repeat(80));
    console.log('\n📊 RESUMEN FINAL:');
    if (result1.passes && result2.passes) {
      console.log('✅ Ambos usuarios deberían verse mutuamente en el feed');
      console.log('⚠️ Si no se ven, puede haber un problema en el código del feed');
    } else {
      console.log('❌ Los usuarios NO se ven mutuamente por las siguientes razones:');
      if (!result1.passes) {
        console.log(`\n  ${chico1.name} → ${chica1.name}:`);
        result1.reasons.filter((r) => r.startsWith('❌')).forEach((r) => console.log(`    ${r}`));
      }
      if (!result2.passes) {
        console.log(`\n  ${chica1.name} → ${chico1.name}:`);
