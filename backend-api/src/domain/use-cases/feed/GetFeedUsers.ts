...
export class GetFeedUsers {
  ...
  async execute(
    ...
    const feedUsersResult = await this.userRepository.findFeedUsers(
      userId,
      limit,
      offset
    );
    ...
    const feedUsers = feedUsersResult.data;
    ...
    for (const user of feedUsers) {
      ...
      if (userAge < preferences.ageMin || userAge > preferences.ageMax)
        continue;
