...
export function getApiUrl(): string {
  ...
  if (!ipMatch) {
    console.log(`[getApiUrl] No IP address detected, using as-is: ${configuredUrl}`);
    return configuredUrl;
  }
  
  const [, ipAddress, port] = ipMatch;
  const localhostUrl = configuredUrl.replace(ipAddressRegex, `http://localhost:${port}`);
  
  // On web, always use localhost instead of IP addresses
  // This avoids CORS and network issues in browsers
  try {
    if (Platform.OS === 'web') {
      console.log(`[getApiUrl] Web platform detected: ${configuredUrl} -> ${localhostUrl}`);
      return localhostUrl;
    }
  } catch (error) {
    // Platform might not be available, check if we're in a browser environment
    if (typeof window !== 'undefined') {
      console.log(`[getApiUrl] Browser environment detected (Platform unavailable): ${configuredUrl} -> ${localhostUrl}`);
      return localhostUrl;
    }
  }
  ...
  try {
    ...
    console.warn('[getApiUrl] Could not determine device status, assuming physical device');
    isSimulator = false;
    deviceDetectionMethod = 'error fallback (assume physical device)';
  }
  
  console.log(`[getApiUrl] Platform: ${Platform.OS}, isSimulator: ${isSimulator} (${deviceDetectionMethod}), configuredUrl: ${configuredUrl}`);
  
  if (isSimulator) {
    // Running in simulator/emulator - can use localhost
    console.log(`[getApiUrl] Simulator/Emulator detected: ${configuredUrl} -> ${localhostUrl}`);
    return localhostUrl;
  }
  
  // Physical device - use configured URL as-is (should be IP address like 192.168.1.11:3000)
  console.log(`[getApiUrl] Physical device detected, using IP: ${configuredUrl}`);
  return configuredUrl;
}
